...
def cart():
    if request.method == 'GET':
        ...
        query = text('''
            SELECT * FROM cart_item, product 
            WHERE cart_item.product = product.id AND cart_item.user = :user_id
        ''')
        
        data = db.session.execute(query, {'user_id': current_user.id})
        print(data)
...
def Payments():
    if request.method == "GET":
        ...
        items = db.session.execute(query, {'user_id': current_user.id})
...
@app.route('/ecommerce/laptop')
@login_required
def Laptops():
    final = Product.query.all()
    print(final)
    return render_template('ecommerce/laptop.html',data=final)

@app.route('/ecommerce/mobile')
@login_required
def Mobiles():
    final = Product.query.all()
    print(final)
    return render_template('ecommerce/mobile.html',data=final)


@app.route('/ecommerce/shop')
@login_required
def Shop():
    final = Product.query.all()
    print(final)
    return render_template('ecommerce/shop.html',data=final)
